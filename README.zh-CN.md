...
KarMind 是一个 Obsidian 插件，用 LLM 帮你把个人资料逐步编译成可持续维护的知识 wiki。它遵循“收集、编译、提问、回填、优化”的工作流，灵感来自 Andrej Karpathy 的 [LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 方法。

如果你想按步骤上手，请阅读 [使用指南](docs/USAGE.zh-CN.md)。
...
1. 把素材放入 `raw/`。网页文章推荐使用 Obsidian Web Clipper 剪藏。
2. 运行 `/compile`，把 raw 素材编译成 wiki 页面。这一步可能消耗大量 LLM token。
...
| 命令 | 说明 |
| --- | --- |
| `/compile` | 把变化过的 raw 笔记编译成 wiki 页面，可能消耗大量 token。 |
...
- 知识管理工作流灵感来自 Andrej Karpathy 的 [LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 原文。
